...
KAGGLE_CONFIG = {
    'dataset_root': '/kaggle/input/casiab-complete',
    ...
    'models': {
        ...
        'p3d': {
            ...
            'iterations': [5000, 10000, 15000, 20000, 25000, 30000, 35000, 40000, 65000, 70000, 75000, 80000, 85000, 90000, 95000, 100000]
        },
        'med-circleloss': {
...
def parse_log_results(log_content):
    ...
    for i, line in enumerate(lines):
        ...
        if 'Rank-1 (Include identical-view cases)' in line:
            if i + 1 < len(lines):
                match = re.search(r'NM:\s+([\d.]+),\s+BG:\s+([\d.]+),\s+CL:\s+([\d.]+)', lines[i + 1])
                if match:
...
def test_single_checkpoint(model_name, iteration, checkpoint_path, dataset_root, partition):
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, 'lib/main.py', '--cfgs', temp_config_path, '--phase', 'test', '--iter', str(iteration)],
            capture_output=True,
            text=True,
            timeout=600
        )
...
def run_batch_test():
    ...
    for model_key, model_info in config['models'].items():
        ...
        for iteration in model_info['iterations']:
            ...
            checkpoint_path = os.path.join(model_info['checkpoint_base'], checkpoint_file)
    ...
    for model_key, iterations in all_results.items():
        ...
        summary_lines.append("-"*100)
        summary_lines.append(f"{'Iter':<10} {'NM_Incl':<10} {'BG_Incl':<10} {'CL_Incl':<10} {'NM_Excl':<10} {'BG_Excl':<10} {'CL_Excl':<10}")
        summary_lines.append("-"*100)
