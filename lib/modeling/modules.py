...
class PackSequenceWrapper(nn.Module):
    ...
    def forward(self, seqs, seqL, seq_dim=1, **kwargs):
        """
            ...
            Out rets: [n, ...]
        """
        # --- ĐOẠN CODE SỬA LỖI: Bóc tách 'options' trước khi xử lý ---
        if 'options' in kwargs:
            opts = kwargs.pop('options')  # Lấy options ra khỏi kwargs
            if 'dim' in opts:
                kwargs['dim'] = opts['dim']  # Đưa dim ra ngoài để hàm max hiểu
        # -------------------------------------------------------------
